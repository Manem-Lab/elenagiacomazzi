...
###############################
## Load Training set
gen_class_disc <- read_csv("Documents/IUCPQ/Data/CHUM_genetics_os_class.csv")
trainingGroup <- factor(c(gen_class_disc$class_os))
matTraining <- gen_class_disc[,!names(gen_class_disc) %in% c("...1", "class_os")]
...
mean(b_acc) 
## 2x25 samples (70%): 0.5434091
## 2x28 samples (80%): 0.5539286
# mean b_acc_ensembles of models in ensemble
mean(b_acc_ensemble) 
## 2x25samples (70%): 0.6725764
## 2x28samples (80%): 0.6908193

# Final ensemble model
selected_model = model[which(b_acc> 0.60)]
length(selected_model) 
## 2x25 samples (70%) uses: 241 models
## 2x28 samples (80%) uses: 347 models
...
save(selected_model, file="Documents/IUCPQ/IUCPQ/R/RData/PCOSP_os.RData")
...
###############################
## Load Validation set
gen_class_vali <- read_csv("Documents/IUCPQ/Data/IUCPQ_genetics_os_class.csv")
testGroup <- factor(c(gen_class_vali$class_os))
matTest <- gen_class_vali[,!names(gen_class_vali) %in% c("...1", "class_os")]

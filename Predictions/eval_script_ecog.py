...
def eval(discovery_iucpq_results_list, feat_sel_meth, nr_sol_list, save_plots=True):
    ...
    for i, nr_sol in enumerate(nr_sol_list):
        ...
        discovery_iucpq_results_list_rename[i] = helper_eval_ecog.rename_for_plotting(discovery_iucpq_results_list[i])
        ...
        helper_data_load.save_csv(discovery_iucpq_results_list_rename[i],file_name1, local, folder="ecog/eval")
        ...
        discovery_iucpq_methods_nr_feat[i] = helper_eval_ecog.get_nr_feat_per_method(discovery_iucpq_results_list_rename[i])
        ...
        helper_data_load.save_csv(discovery_iucpq_methods_nr_feat[i], file_name2, local, folder="ecog/eval")
    
        # Build a dict which maps the method to the nr of features for best performance
        dicovery_iucpq_meth_nr_feat_dict[i] = helper_eval_ecog.get_meth_feat_dict(discovery_iucpq_methods_nr_feat[i], random_state=random_state)
        
        # Train all methods with best nr_feat on the whole discovery dataset and validate on validation set
        df_vali_on_chum[i] = helper_eval_ecog.train_disc_n_validate(data_norm_iucpq, data_norm_chum, dicovery_iucpq_meth_nr_feat_dict[i], feat_sel_meth, nr_sol, "CHUM")
...
def main_eval(feat_sel_method, corr_meth="spearman", random_state=42):
    ...
    if feat_sel_method == "mrmr":
        ...
        for nr_sol in nr_sol_list:
            result_discovery_IUCPQ = helper_data_load.load_csv(file_name="os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", folder="ecog/results", local=local)
        ...
        result_discovery_iucpq_list.append(helper_data_load.load_csv(file_name="os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", folder="ecog/results", local=local))
    else:
        nr_sol_list = [-1]
        nr_sol = nr_sol_list[0]
        result_discovery_iucpq_list.append(helper_data_load.load_csv("os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", folder="ecog/results", local=local))

...
def start_pred(data_norm, random_state, feat_sel_method, nr_sol_list):
    ...
    for nr_sol in nr_sol_list:
        ...
        result_discovery_IUCPQ_all = helper.all_methods(data_norm, random_state, FEAT_SEL_METH=feat_sel_method, NR_SOL=nr_sol)
        ...
        bucket = 'oncotechdata'
        result_discovery_IUCPQ_all.to_csv(f"s3://{bucket}/results/classification/os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", storage_options={"client_kwargs": {'endpoint_url': ENDPOINT_URL}})
        os.system("echo saved df for feat_sel_meth {} and nr_sol {}".format(feat_sel_method, nr_sol))

def main(feat_sel_method, random_state=42, nr_sol_temp=5):
    os.system("echo in main with {}".format(feat_sel_method))
    data_norm_chum, data_norm_iucpq = helper_load.get_norm_data(local=False, classification=True)
    ...
    #list_corr_meth = ["pearson", "spearman"] # , "spearman"

    #for feat_sel_method in list_feat_sel_methods:
    if feat_sel_method == "corr":
        for corr_meth in list_corr_meth:
            ...
            start_pred(data_norm_iucpq, random_state, feat_sel_method, nr_sol_list)
...
parser.add_argument("-ns", "--NrSol", help = "Add number of solutions used by mrmr method", default=5)

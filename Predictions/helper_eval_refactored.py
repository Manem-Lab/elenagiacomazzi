...
#### Train Model
def train_final_model_test_on_validation_cohort(model, predictor, train_df, test_df, feat_sel_meth, nr_feat, random_state=42, ensemble=False, nr_solutions=0):
    X_train = train_df.loc[:, train_df.columns != predictor]
    y_train = train_df.loc[:, predictor]

    X_test = test_df.loc[:, test_df.columns != predictor]
    y_test = test_df.loc[:, predictor]
    ...
    if ensemble:
        ...
        for nr_f in range(1, nr_feat+1):
            ...
            model, y_pred = helper.select_feats(feat_sel_meth, X_train, y_train, X_test, model, nr_f, nr_solutions, random_state, predictor)
            pred_list.append(y_pred) # should have len of nr_feat
        
        preds_array = np.array(pred_list)
        y_predict = np.average(preds_array, axis = 0)
    else:
        model, y_predict = helper.select_feats(feat_sel_meth, X_train, y_train, X_test, model, nr_feat, nr_solutions, random_state, predictor)
...
def train_disc_n_validate(predictor, train_df, test_df, methods_dict, feat_sel_meth, nr_sol, val_set, k_max, local, savePath):
    ensemble = False
    ...
    for i in range(len(methods_dict)):
        ...
        if method_temp == "Ensemble Lin. R.":
            ensemble = True
        c_idx_tested_on_val_set, y_pred_vali = train_final_model_test_on_validation_cohort(method_temp, predictor, train_df, test_df, feat_sel_meth, feat_nr_temp, ensemble=ensemble, nr_solutions=nr_sol)
    ...
    file_name = predictor + "_preds_"+ feat_sel_meth+str(nr_sol)+ "on_vali_"+val_set+".csv"
    #df, file_name, local, savePath, folder
    helper_data_load.save_csv(df_predictions_vali, file_name, local, savePath, "/eval/preds/")

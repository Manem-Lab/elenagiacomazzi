...
def fit_and_predict(model, X_train, y_train, feat_to_use, X_test, random_state, feat_sel_meth):
    ...
    if feat_sel_meth not in ["mim", "f_reg"]:
...
def kFold_training(k_fold_nr, random_state, model, data_norm, nr_feat, ensemble=False, feat_sel_method="corr", nr_solutions=0, save_pred=False):
    ...
    if ensemble: 
        ...
        if save_pred:
            ...
            print(file_name)
            helper_data_load.save_csv(df_preds, file_name=file_name, local=local, folder="results/predictions")            #df_preds.to_csv("../../Data/Results/test_folder_pred/" + "cv_preds_"+str(model)+"_"+feat_sel_method+str(nr_solutions))
        else:
...
def lin_reg_ensamble(data_norm, random_state, feat_sel_meth="corr", nr_sol=0):
    ...
    helper_data_load.save_csv(df_preds, file_name=file_name, local=local, folder="results/predictions") 

    return df_result_ens

def not_ensemble_methods(method, df_result, data_norm, random_state, feat_sel_meth="corr", nr_sol=0, k_max=None):
    if k_max == None:

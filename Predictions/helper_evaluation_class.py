...
import helper_prediction_class as helper_class
...
if local:
    ...
    folder = "eval/classification/preds"
...
def rename_for_plotting(result_nr_df):
    list_methods_naming = ["Nr. Features", "Ensemble R.", "Logistic R.", "Random Forest C.", "SVC", "DecisionTree C.", "knn C.", "Gaussian C.", "Bagging C.", "GradiendBoosting C."]
...
def train_final_model_test_on_validation_cohort(model, train_df, test_df, feat_sel_meth, nr_feat, random_state=42, ensemble=False, nr_solutions=0):
    ...
    X_train = train_df.loc[:, train_df.columns != 'class_os_days']
    y_train = train_df.loc[:, "class_os_days"]

    X_test = test_df.loc[:, test_df.columns != 'class_os_days']
    y_test = test_df.loc[:, "class_os_days"]
    ...
    if ensemble:
        ...
        for nr_f in range(1, nr_feat+1):
            ...
            model, y_pred = helper_class.select_feats(feat_sel_meth, X_train, y_train, X_test, model, nr_f, nr_solutions, random_state)
            pred_list.append(y_pred) # should have len of nr_feat
        
        preds_array = np.array(pred_list)
        y_predict = np.average(preds_array, axis = 0)
    else:
        model, y_predict = helper_class.select_feats(feat_sel_meth, X_train, y_train, X_test, model, nr_feat, nr_solutions, random_state)
...
def get_meth_feat_dict(df_dicovery_mthod_nr_feat, random_state):
    meth_feat_dict = {}
    dict_map_str_method = {"SVC": SVC(kernel="linear"), "Logistic R.": LogisticRegression(), "Random Forest C.": RandomForestClassifier(n_estimators=100, random_state=random_state), "Bagging C.": BaggingClassifier(random_state=random_state),
                            "GradiendBoosting C.": GradientBoostingClassifier(random_state=random_state), "Gaussian C.": GaussianNB(),
                            "Ensemble R.": "Ensemble Log. R.", "knn C.": KNeighborsClassifier(), "DecisionTree C.":DecisionTreeClassifier(random_state=random_state)}
...
def prep_method_name(df, random_state, name_to_func):
    ...
    dict_map_str_method = {"SVC": "SVC(kernel='linear')", "Logistic R.": "LogisticRegression()", "Random Forest C.": "RandomForestClassifier(random_state="+random_state+")", "Bagging C.": "BaggingClassifier(random_state="+random_state+")",
                                "GradiendBoosting C.": "GradientBoostingClassifier(random_state="+random_state+")", "Gaussian C.": "GaussianNB()",
                                "Ensemble R.": "Ensemble Log. R.", "knn C.": "KNeighborsClassifier()", "DecisionTreeClassifier(random_state="+random_state+")":"DecisionTree C."}
    if name_to_func:
...
def plot_vali_results(df_vali_on, discovery_methods_nr_feat, val_set, feat_sel_method, nr_sol, random_state, save_plots=False):
    ...
    if save_plots:
        if local:
            ...
            fig.savefig('../../Plots/class_os_days_validation_on_' + val_set +'_'+feat_sel_method+str(nr_sol)+'_1-100.png', dpi=150, bbox_inches = 'tight')  
        else:
            s3 = boto3.resource('s3', aws_access_key_id= 'YOUR_ACCESS_KEY_ID', aws_secret_access_key='YOUR_SECRET_ACCESS_KEY')
            ENDPOINT_URL = 'https://s3.valeria.science'
            bucket = 'oncotechdata'
            fig = vali_plot.get_figure()
            fig.savefig(f's3://{bucket}/plots/class_os_days_validation_on_' + val_set +'_'+feat_sel_method+str(nr_sol)+'_1-100.png', dpi=150, bbox_inches = 'tight', storage_options={"client_kwargs": {'endpoint_url': ENDPOINT_URL}})  
...
def plot_vali_results_in_one(df_vali_on_1, df_vali_on_2, df_disc_on_1, df_disc_on_2, feat_sel_method, nr_sol, save_plots=False):
    ...
    if save_plots:
        if local:
            fig = vali_plot.get_figure()
            fig.savefig('../../Plots/class_os_days_validation_both_in_one_plot_'+feat_sel_method+str(nr_sol)+'_1-100.png', dpi=150, bbox_inches = 'tight')  
        else:
            s3 = boto3.resource('s3', aws_access_key_id= 'YOUR_ACCESS_KEY_ID', aws_secret_access_key='YOUR_SECRET_ACCESS_KEY')
            ENDPOINT_URL = 'https://s3.valeria.science'
            bucket = 'oncotechdata'
            fig = vali_plot.get_figure()
            fig.savefig(f's3://{bucket}/plots/class_os_days_validation_both_in_one_plot_'+feat_sel_method+str(nr_sol)+'_1-100.png', dpi=150, bbox_inches = 'tight', storage_options={"client_kwargs": {'endpoint_url': ENDPOINT_URL}})  

...
import helper_prediction_ecog as helper
...
def start_pred(data_norm, random_state, feat_sel_method, nr_sol_list):
    ...
    for nr_sol in nr_sol_list:
        ...
        bucket = 'oncotechdata'
        result_discovery_IUCPQ_all.to_csv(f"s3://{bucket}/ecog/results/os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", storage_options={"client_kwargs": {'endpoint_url': ENDPOINT_URL}})
    ...
    import eval_script_ecog as eval
    eval.eval_script_ecog.main(feat_sel_method, corr_meth=corr_meth, random_state=42)
    print("EVALUATION finished")


def main(feat_sel_method, random_state=42, nr_sol_temp=1):
    os.system("echo in main with {}".format(feat_sel_method))
    data_norm_chum, data_norm_iucpq = helper_load.get_norm_data(local=False, ecog=True)

...
def eval(discovery_chum_results_list, feat_sel_meth, nr_sol_list, save_plots=True):
    ...
    data_norm_chum, data_norm_iucpq = helper_load.get_norm_data(local=local)
    for i, nr_sol in enumerate(nr_sol_list):
        #### Discovery == CHUM, Validation == IUCPQ ####
        # rename columns for plotting
        print(i)
        discovery_chum_results_list_rename[i] = helper_eval.rename_for_plotting(discovery_chum_results_list[i])
        ...
        file_name1 = 'os_days_disc_CHUM_Bagging_renamed_1-100_'+feat_sel_meth+str(nr_sol)+'.csv'
        helper_data_load.save_csv(discovery_chum_results_list_rename[i],file_name1, local, folder="eval")
        print("Feat sel {} nr_sol {}, saved {}".format(feat_sel_meth, nr_sol, file_name1))
        ...
        file_name2 = "os_days_CHUM_discovery_Bagging_nr_feat"+"_"+feat_sel_meth+str(nr_sol)+"_1-100.csv"
        helper_data_load.save_csv(discovery_chum_methods_nr_feat[i], file_name2, local, folder="eval")
        print("Feat sel {} nr_sol {}, saved {}".format(feat_sel_meth, nr_sol, file_name2))
        ...
        df_vali_on_chum[i] = helper_eval.train_disc_n_validate(data_norm_chum, data_norm_iucpq, dicovery_chum_meth_nr_feat_dict[i], feat_sel_meth, nr_sol, val_set= "IUCPQ")
        df_vali_on_chum[i].set_index("Method Name", inplace=True)
        # Save the validation results and plot them
        file_name3 = "os_days_results_best_model_validation_IUCPQ_Bagging_"+feat_sel_meth+str(nr_sol)+"_1-100.csv"
        helper_data_load.save_csv(df_vali_on_chum[i], file_name3, local, folder="eval/c_idxs")
        print("Feat sel {} nr_sol {}, saved {}".format(feat_sel_meth, nr_sol, file_name3))
        print("EVALUATION OVER")
        # Plot performance of dicovery and validation set for each method
        ### helper_eval.plot_vali_results(df_vali_on_chum[i], discovery_chum_methods_nr_feat[i], val_set="CHUM", feat_sel_method=feat_sel_meth, nr_sol=nr_sol, random_state=random_state, save_plots=save_plots)
...
def main_eval(feat_sel_method, corr_meth="spearman", random_state=42):
    ...
    if feat_sel_method == "mrmr":
        ...
        for nr_sol in nr_sol_list:
            result_discovery_chum = helper_data_load.load_csv(file_name="os_days_results_CHUM_discovery_Bagging_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", local=local)
    ...
    if feat_sel_method == "corr1":
        ...
        result_discovery_chum_list.append(helper_data_load.load_csv(file_name="os_days_results_CHUM_discovery_Bagging_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", local=local))
        ...
        nr_sol_list = [-1]
        nr_sol = nr_sol_list[0]
        result_discovery_chum_list.append(helper_data_load.load_csv("os_days_results_CHUM_discovery_Bagging_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", local))

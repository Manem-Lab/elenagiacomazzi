...
def eval(discovery_iucpq_results_list, feat_sel_meth, nr_sol_list, save_plots=True):
    ...
    for i, nr_sol in enumerate(nr_sol_list):
        ...
        helper_data_load.save_csv(discovery_iucpq_results_list_rename[i],file_name1, local, folder="eval")
        ...
        helper_data_load.save_csv(discovery_iucpq_methods_nr_feat[i], file_name2, local, folder="eval")
...
def main(feat_sel_method, corr_meth="spearman", random_state=42):
    ...
    if feat_sel_method == "mrmr":
        ...
        for nr_sol in nr_sol_list:
            result_discovery_IUCPQ = helper_data_load.load_csv(file_name="os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", local=local)
        ...
        result_discovery_iucpq_list.append(helper_data_load.load_csv(file_name="os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", local=local))
    else:
        nr_sol_list = [-1]
        nr_sol = nr_sol_list[0]
        result_discovery_iucpq_list.append(helper_data_load.load_csv("os_days_results_IUCPQ_discovery_"+feat_sel_method+str(nr_sol)+"_std_scl_1-100.csv", local))
...
#main(args.FeatureSelection, args.CorrMeth, random_state)
